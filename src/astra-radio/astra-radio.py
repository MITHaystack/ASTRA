"""
    ...
    This software service controls the software radio, manages ringbuffers and snapshots, and produces
    related MQTT telemetry. It accepts commands via MQTT to control the operating state and implement
    radio control behaviors. 
...
class AstraAntennaState:
    def __init__(self):
        ...
        self.target = {
            'group'          : 'mount-target',
            'target_type'   : 'fixed', # fixed, scan, solar, lunar, radec, SGP4, Horizons
...
async def antenna_motion_handler(options, mount, mount_lock, antenna_state, antenna_pointing, eventQ, telemetryQ, logQ, motion_period):
    ...
    event_cnt = 0
    target_state = 'none' # none, on_target, goto_target, scan, track_target, track_object, track_radec, stop, quick_stop
    on_target = True 
    update_mount = False
        ...
        try:
            ...
            if cmd is not None:
                match cmd['event']:
                    ...
                    case 'antenna-stop':
                        pass
                    case 'antenna-track-object':
                        pass
                    case 'antenna-track-radec':
                        pass
                    case 'antenna-offset':
                        pass
