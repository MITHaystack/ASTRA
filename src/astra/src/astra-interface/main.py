        ...
        try:
            ...
            try:
                async with mqtt_client:
                    grp = tdata['group']
                    await mqtt_client.publish(f"astra/ai/telemetry/{grp}", payload=tdata_json.encode('utf-8'))
                    if options.verbose:
            ...
            status = {'event':'telemetry', 'group':'astra-interface', 'source':'telemetry_mqtt_handler', 'value':f"dispatched {tdata['event']}"}
            await logQ.put(status)
    ...
    while True:
        ...
        try:
            ...
            try:
                async with mqtt_client:
                    grp = sdata['group']
                    await mqtt_client.publish(f"astra/ai/state/{grp}", payload=sdata_json.encode('utf-8'))
                    if options.verbose:
            ...
            status = {'event':'status', 'group':'astra-interface', 'source':'state_mqtt_handler', 'value':f"dispatched {sdata['event']}"}
            await logQ.put(status)
...
"""
    Log info from the serial handler is routed to the MQTT 'astra/log/<event type>' channel. 
    ...
    while True:
        ...
        try:
            ...
            try:
                async with mqtt_client:
                    grp = ldata['group']
                    await mqtt_client.publish(f"astra/ai/log/{grp}", payload=ldata_json.encode('utf-8'))
                    if options.verbose:
                ...
                status = {'event':'exception', 'group':'astra-interface','source':'log_mqtt_handler', 'value':f"mqtt connection lost {err}"}
                await logQ.put(status)
            ...
            status = {'event':'status', 'group':'astra-interface', 'source':'log_mqtt_handler', 'value':f"dispatched {ldata['event']}"}
            await logQ.put(status)
        ...
        try:
            async with aiomqtt.Client(hostname=options.mqtt, port=1883) as mqtt_client:
                ...
                try:
                    ...
                    status = {'event':'exception', 'group':'astra-interface', 'source':'command_mqtt_handler', 'value':f"mqtt connection lost {err}"}
                    await logQ.put(status)
                    continue
                except json.JSONDecodeError as err:
                    if options.verbose:
                        print(f"Problem decoding json command object, {cmd}")
                    status = {'event':'exception', 'group':'astra-interface', 'source':'command_mqtt_handler', 'value':f"json decode problem {err}"}
                    await logQ.put(status)
                    continue
                except Exception as err:
                    print(f"exception in command handler {err}")
   
                status = {'event':'log', 'group':'astra-interface', 'source':'command_mqtt_handler', 'value':f"dispatched {cmd}"}
                await logQ.put(status)
