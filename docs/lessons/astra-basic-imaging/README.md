...
## Safety
While the sun is by far the easiest and brightest object to point at, and also one of the only ones able to be seen during the day, it is also not without risk. Looking at the sun without proper filtering can and will cause eye damage. Pointing a camera at the sun without proper filtering can also harm the camera. Before pointing your ASTRA unit close to the sun, make sure that it has a good solar filter on the optical telescope.
...
{image of taped solar filter}

Go to the Control page of the GUI. Select "Sun" from the Source catalog and press "GOTO SOURCE".
![img](GoToSun.png)
